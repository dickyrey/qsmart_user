...
import '../model/usernew_model.dart';
...
//----------------login custom------------------------------------------------
  ...
  Future<UserNew> loginatorapi(String id, String pass) async {
    ...
    var cek2 = await ref.child('admin/$id').once();
    print(cek2.value);
    if (cek2.value != null) {
...
//-----------------user admin------------------------------------
  ...
  Future<void> deletUser(String id) async {
    return ref
        .child('user/$id')
        .remove()
        .then((_) => print('remove user $id suksess isi dua'));
  }
  ...
  Future<List<Listbanksoal>> cariSoal(String kelas, String mapel) async {
    print('cari soal');
    var getdata = await ref
        .child('carisoal/$kelas/$mapel')
  ...
  Future<List<Soalnye>> getSoalnye(String idsoal) async {
    return ref.child('banksoal/$idsoal/soalnye').once().then((onValue) {
      return List<Soalnye>.from(
          onValue.value.map((x) => x == null ? null : Soalnye.fromJson(x)));
    });
  }
...
//--------------------------------------------------materi-------------------------------------------
  ...
  Future<List<MateriModel>> getListMateriAdmin(
      String tingkat, String mapel) async {
  ...
  Future<void> buatmateri(MateriModel data) async {
    await ref
        .child('materi/${data.tingkat}/${data.mapel}')
        .push()
        .set(data.toMap());
  }
...
//------------------------------------------------------------admin----------------------------------
  ...
  Future<void> hapusPaketSoal(Listbanksoal data) async {
    await ref
        .child('banksoal/${data.id}')
        .remove()
        .then((_) => print('hapus paket soal sukses'))
        .then((_) async {
      await ref
          .child('carisoal/${data.kelas}/${data.mapel}/${data.id}')
          .remove();
...
//----------------------buat soal v2-------------------------------------------------------------
  ...
  Future<void> ngetesBuatSoalLangsungPushtapiFalse(
  ...
  Future<void> publishSoaltrueV2(
      {String kelas, String mapel, String idsoal}) async {
  ...
  Future<void> unPublishSoalV2(String kelas, String mapel, String id) async {
    return ref.child('carisoal/$kelas/$mapel/$id/published')
      ..set(false).then((_) async {
        await ref
            .child('banksoal/$id/published')
            .set(false)
            .then((_) => print('unpublish sukses'));
